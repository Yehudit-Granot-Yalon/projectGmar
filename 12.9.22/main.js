...
txtFile.onload = function(e) {
  // document.getElementById("map").innerHTML = txtFile.responseText; 
var file=txtFile.responseText
...
txtFile.open("GET", "a1-30m-DEM.tif", true); 
txtFile.send(null);
console.log("1");
...
var height=set_height(row[9]);
feature.setProperties({ "a": row[2], "b": row[3], "c": row[4], "d": row[5], "e": row[6],"f": row[7],"g": row[8],"h":  row[9],"i": row[10],"j": row[11],"k": row[12],"l": row[13],"m": row[14],"n": row[15],"o": row[16],"p": row[17],"q": row[18],"r":row[19],"s":lat,"t":lon,"u":height})
//feature.setProperties({ "x": lon, "y": lat, "height": 1, "velocity": 98, "direction": 80})
