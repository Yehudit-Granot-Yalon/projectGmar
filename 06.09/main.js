   ...
   wb.xlsx.load(buffer).then(workbook => {
    console.log(workbook, 'workbook instance')
     workbook.eachSheet((sheet, id) => {
       ...
       if(rowIndex>=7&&row.values[2]!=undefined){
...
console.log(row[20]);
feature.setProperties({ "a": row[2], "b": row[3], "c": row[4], "d": row[5], "e": row[6],"f": row[7],"g": row[8],"h":  row[9],"i": row[10],"j": row[11],"k": row[12],"l": row[13],"m": row[14],"n": row[15],"o": row[16],"p": row[17],"q": row[18],"r":row[19],"s":lon,"t":lat,"u":height})
